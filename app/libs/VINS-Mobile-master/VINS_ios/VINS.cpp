...
//TODO: Remove debugging help: (also checkpoints in code)
...
void VINS::clearState() {
    ...
    for (int i = 0; i < 10 * (WINDOW_SIZE + 1); i++) {
        ...
        // Why??? it's set to null 4 lines ago ... without destructing the object...
        // TODO: leak?
        if (pre_integrations[i] != nullptr) {
            printf("checkpoint call delete on an IntegrationBase object");
    ...
    if (tmp_pre_integration != nullptr) {
        printf("Why is tmp_pre_integration not nullptr?");
    ...
    if (LOOP_CLOSURE && loop_enable) {
        ...
        for (int i = 0; i < WINDOW_SIZE; i++) {
            if (front_pose.header == Headers[i]) {
                Matrix3d Rs_loop = Quaterniond(front_pose.loop_pose[6], front_pose.loop_pose[3],
                                               front_pose.loop_pose[4],
                                               front_pose.loop_pose[5]).normalized().toRotationMatrix();
                Vector3d Ps_loop = Vector3d(front_pose.loop_pose[0], front_pose.loop_pose[1],
                                            front_pose.loop_pose[2]);

                Rs_loop = rot_diff * Rs_loop;
                Ps_loop = rot_diff *
                          (Ps_loop - Vector3d(para_Pose[0][0], para_Pose[0][1], para_Pose[0][2])) +
                          origin_P0;
...
bool VINS::failureDetection() {
    ...
    Vector3d trans = (tmp_P - last_P);
    if (trans.norm() > 1) {
        printf("failure big translation: %lf %lf %lf\n", trans.x(), trans.y(), trans.z());
        is_failure = true;
    }
...
void VINS::processImage(map<int, Vector3d> &image_msg, double header, int buf_num) {
    ...
    if (solver_flag == INITIAL) {
        ...
        tmp_pre_integration = new IntegrationBase{acc_0, gyr_0, Vector3d(0, 0, 0),
                                                  Vector3d(0, 0, 0)};
...
void VINS::solve_ceres(int buf_num) {
            ...
            if (front_pose.header >= Headers[0]) {
                ...
                for (int i = 0; i < WINDOW_SIZE; i++) {
                    if (front_pose.header == Headers[i]) {
                        ...
                        ceres::LocalParameterization *local_parameterization = new PoseLocalParameterization();
                        problem.AddParameterBlock(front_pose.loop_pose, SIZE_POSE,
                        ...
                        for (auto &it_per_id : f_manager.feature) {
                            ...
                            if (start <= i && end >= 0) {
                                while (front_pose.features_ids[retrive_feature_index] <
                                       it_per_id.feature_id) {
                                    retrive_feature_index++;
                                }

                                if (front_pose.features_ids[retrive_feature_index] ==
                                    it_per_id.feature_id) {
                                    Vector3d pts_j = Vector3d(
                                            front_pose.measurements[retrive_feature_index].x,
                                            front_pose.measurements[retrive_feature_index].y, 1.0);
                                    Vector3d pts_i = it_per_id.feature_per_frame[0].point;
                                    //double ratio = 1.0;
                                    ProjectionFactor *f = new ProjectionFactor(pts_i, pts_j);
                                    problem.AddResidualBlock(f, loss_function, para_Pose[start],
                                                             front_pose.loop_pose, para_Ex_Pose[0],
    ...
    if (LOOP_CLOSURE) {
        for (int i = 0; i < WINDOW_SIZE; i++) {
            if (front_pose.header == Headers[i]) {
                Matrix3d Rs_i = Quaterniond(para_Pose[i][6], para_Pose[i][3], para_Pose[i][4],
                                            para_Pose[i][5]).normalized().toRotationMatrix();
                ...
                front_pose.relative_yaw = Utility::normalizeAngle(
                        Utility::R2ypr(Rs_i).x() - Utility::R2ypr(Rs_loop).x());
            }
        }
    }
...
bool VINS::visualInitialAlign() {
    ...
    printf("PS after scale s = %d: %lf %lf %lf\n", s, Ps[0].x(), Ps[0].y(), Ps[0].z());
