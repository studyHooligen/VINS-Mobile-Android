...
//            TS(smallWindow_into_big);
            cv::Mat imageROI;
            imageROI = tmp2(cv::Rect(10, COL - down_origin_image.rows - 10, down_origin_image.cols,
...
//                        Vector3d pts_i = it_per_id.estimated_depth * it_per_id.feature_per_frame[0].point;
                ...
                tmp_feature.position =
                        vins.r_drift * vins.Rs[imu_i] * (vins.ric * pts_i + vins.tic) +
        ...
        if (LOOP_CLOSURE) {
            ...
            if (vins.marginalization_flag == vins.MARGIN_OLD &&
                vins.solver_flag == vins.NON_LINEAR && !image_buf_loop.empty()) {
                first_frame = false;
...
//        LOGI("THREAD: loop_thread(loopDetectionLoop)");
        if (!LOOP_CLOSURE) {
            std::this_thread::sleep_for(
                    std::chrono::milliseconds(500)); // [NSThread sleepForTimeInterval:0.5];
            continue;
        }
        ...
        if (loop_succ)
            std::this_thread::sleep_for(
                    std::chrono::seconds(2)); // [NSThread sleepForTimeInterval:2.0];
        std::this_thread::sleep_for(
                std::chrono::milliseconds(50)); // [NSThread sleepForTimeInterval:0.05];
    }
...
//        LOGI("THREAD: globalLoopThread(globalPoseGraphLoop)");
        if (start_global_optimization) {
            ...
            TE(globalPoseGraph);
            std::this_thread::sleep_for(
                    std::chrono::milliseconds(1170)); // [NSThread sleepForTimeInterval:1.17];
        }
        std::this_thread::sleep_for(
                std::chrono::milliseconds(30)); // [NSThread sleepForTimeInterval:0.03];
    }
}
...
//            }
        std::this_thread::sleep_for(
                std::chrono::milliseconds(40)); // [NSThread sleepForTimeInterval:0.04];
    }
}
    ...
    if (vins.solver_flag != vins.NON_LINEAR) {
        ...
        switch (vins.init_status) {
            case VINS::InitStatus::FAIL_IMU: // vins.FAIL_IMU:
                oss
                        << "STA: FAIL_IMU"; // stringView = [NSString stringWithFormat:@"STA: FAIL_IMU"];
                break;
            case VINS::InitStatus::FAIL_PARALLAX:
                oss
                        << "STA: FAIL_PARA"; // stringView =[NSString stringWithFormat:@"STA: FAIL_PARA"];
                break;
            case VINS::InitStatus::FAIL_RELATIVE:
                oss
                        << "STA: FAIL_RELA"; // stringView =[NSString stringWithFormat:@"STA: FAIL_RELA"];
                break;
            ...
            case VINS::InitStatus::FAIL_ALIGN:
                oss
                        << "STA: FAIL_ALIGN"; // stringView =[NSString stringWithFormat:@"STA: FAIL_ALIGN"];
                break;
            case VINS::InitStatus::FAIL_CHECK:
                oss
                        << "STA: FAIL_COST"; // stringView =[NSString stringWithFormat:@"STA: FAIL_COST"];
                break;
...
//        env->CallVoidMethod(tvX, setTextMethod, tempJStr); // [_X_label setText:stringView];
        ...
        oss << "FAIL: " << vins.fail_times
            << " times"; // stringView =[NSString stringWithFormat:@"FAIL: %d times", vins.fail_times];
//        env->CallVoidMethod(tvY, setTextMethod, env->NewStringUTF(oss.str().c_str())); // [_Y_label setText:stringView];
        ...
        oss << "PARALLAX: "
            << vins.parallax_num_view; // stringView =[NSString stringWithFormat:@"PARALLAX: %d", vins.parallax_num_view];
//        env->CallVoidMethod(tvZ, setTextMethod, env->NewStringUTF(oss.str().c_str())); // [_Z_label setText:stringView];
        ...
        oss << "Initializing: "
            << vins.initProgress; // stringView =[NSString stringWithFormat:@"Initializing: %d%%", vins.initProgress];
...
//        env->CallVoidMethod(tvX, setTextMethod, env->NewStringUTF(oss.str().c_str())); // [_X_label setText:stringView];
        ...
        oss << "TOTAL: "
            << total_odom; // stringView =[NSString stringWithFormat:@"TOTAL:%.2f", total_odom];
...
//        env->CallVoidMethod(tvZ, setTextMethod, env->NewStringUTF(oss.str().c_str())); // [_Z_label setText:stringView];
    ...
    oss << "BUF: "
        << waiting_lists; // stringView =[NSString stringWithFormat:@"BUF:%d", waiting_lists];
//    env->CallVoidMethod(tvBuf, setTextMethod, env->NewStringUTF(oss.str().c_str())); // [_buf_label setText:stringView];
    ...
    if (loop_old_index != -1) {
        oss << "LOOP with: "
            << loop_old_index; // stringView =[NSString stringWithFormat:@"LOOP with %d", loop_old_index];
//        env->CallVoidMethod(tvLoop, setTextMethod, env->NewStringUTF(oss.str().c_str())); // [_loop_label setText:stringView];
    ...
    oss << "FEATURE: "
        << vins.feature_num; // stringView =[NSString stringWithFormat:@"FEATURE: %d", vins.feature_num];
